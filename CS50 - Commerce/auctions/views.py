...
def listing_page(request, auction_id):
    ...
    # Current auction if exists
    try:
        auction = Auction.objects.get(pk=auction_id)
    except Auction.DoesNotExist:
        return render(request, "auctions/error.html", {
            "code": 404,
            "message": "This auction not exist"
        })
    ...
    # Show auction only to the winner and the seller if it's closed
    if auction.closed:
        ...
        return HttpResponse("Error - auction no longer available")
         ...
         # User logged in, check if auction already in watchlist
...
def watchlist(request):
    ...
    # Saved info about the auction and go back to auction's page
    if request.method == "POST":
        # About the auction
        auction_id = request.POST.get("auction_id")

        # If that auction exists
        try:
            auction = Auction.objects.get(pk=auction_id)
            user = User.objects.get(id=request.user.id)
        except Auction.DoesNotExist:
            return render(request, "auctions/error.html", {
                "code": 404,
                "message": "This auctions not exist"
            })
        ...
        if request.POST.get("on_watchlist") == "True":
            ...
            try:
                ...
                watchlist_item.save()
            # Check if the auction not exist already for this user
...
def bid(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Check if the bid_price is positive
            if bid_price <= 0:
                return render(request, "auctions/error.html", {
                    "code": 400,
            ...
            # Check if that bid is not made by the seller
            ...
            # Check if the latest bid is the highest / else save new bid
            ...
            if highest_bid is None or bid_price > highest_bid.bid_price:
                ...
                return render(request, "auctions/error.html", {
                    "code": 400,
                    "message": "You're bid is too small"
                })
        else:
            return render(request, "auctions/error.html", {
                "code": 400,
