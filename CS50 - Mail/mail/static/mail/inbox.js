        ...
        .then(email => {
            ...
            ["subject", "timestamp", "sender", "recipients", "body"].forEach(email_element => {
                ...
                if (email_element === "subject") {
                    // Subject section with two buttons for reply and archive
                    ...
                    // We don't need the archive button, if the mailbox we came from was "sent".
                    (origin_mailbox === "sent" ?
  ...
  fetch('/emails', {
    ...
    .then(response => response.json())
      .then(result => {
        ...
        if ("error" in result) {
            // Error in sending the email? Display the error next to the "To:"
